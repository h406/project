...
void MeshCylinder::initVtx() {
  ...
  // 半径の計算
  float	cylinderVtxX = 0 - ((_nNumBlockX * _fSizeBlockX)) / 2;
  float	cylinderVtxY = 0 - ((_nNumBlockY * _fSizeBlockY)) / 2;
  ...
  for (float cntZ = 0.0f; cntZ <= _nNumBlockY; cntZ++)
  {
    for (float cntX = 0.0f; cntX <= _nNumBlockX; cntX++)
    {
      ...
      float	length = (_nNumBlockX * _fSizeBlockX) / 2;
    ...
    // 座標始点Xを戻す
    cylinderVtxX = 0 - (_nNumBlockX  * _fSizeBlockX) / 2;
  }
